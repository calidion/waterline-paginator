...
export default {
  ...
  paginate: function (data, cb) {
    ...
    var limit = data.limit ? data.limit : process.env.WATERLINE_PAGINATOR_LIMIT;
    var offset = (data.page - 1) * limit;

    var one = query.limit(limit).skip(offset);
    var all = data.model.count(data.conditions);

    Promise.all([one, all]).then(function (results, countValue) {
      if (countValue) {
        var total = parseInt(countValue / limit, 10);
        if ((countValue % limit) !== 0) {
          total++;
